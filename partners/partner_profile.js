const API = "https://script.google.com/macros/s/AKfycbwtClA1mmRpEO154x20nahYGhDAN83ODXPE1jhzJs65aqkCnMEldsmwFoTyTF44Rp3j/exec";
...
// 2. UI DIKHANE WALA FUNCTION (ISME BUTTON FIX HAI)
function renderProfileUI(p) {
    ...
    // Badge Colors Logic
    let badgeColor = "#ffcc00"; // Default Yellow
    ...
    else if (s === "ON HOLD") badgeColor = "#ff9900";

    // SECURITY: Agar Blacklisted hai to ye dikhao aur return kar jao
    if (s === "BLACKLISTED") {
        mainArea.innerHTML = `
                ...
                <form id="profileForm">
                    ...
                    <div style="text-align: center; margin-bottom: 40px;">
                        <div style="position: relative; display: inline-block;">
                            <img src="${photoUrl}" id="profile-img-preview" 
                                 style="width: 140px; height: 140px; border-radius: 50%; border: 4px solid var(--primary-teal); object-fit: cover; object-position: center; aspect-ratio: 1/1; display: block; padding: 5px; background: rgba(255,255,255,0.05); box-shadow: 0 8px 20px rgba(0,0,0,0.4);">
                        ...
                        <div style="margin-top: 5px;">
                            <span style="background: ${badgeColor}33; color: ${badgeColor}; padding: 4px 15px; border-radius: 20px; font-size: 11px; font-weight: bold; border: 1px solid ${badgeColor}; text-transform: uppercase;">● ${s}</span>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px;">
                        ...
                        <div>
                            ...
                            <input type="text" class="btn btn-outline" style="width:100%; text-align:left; background: rgba(255,255,255,0.02); color: var(--text-dim);" value="${p.email || localStorage.getItem('userEmail') || ''}" readonly>
                        ...
                        <div>
                            ...
                            <input type="text" class="btn btn-outline" style="width:100%; text-align:left; border: 1px dashed var(--secondary-gold); color: var(--secondary-gold); font-weight: bold;" value="${(s === 'ACTIVE' || s === 'APPROVED') ? (p.referralCode || 'NOT ASSIGNED') : 'Unlock After Admin Approval'}" readonly>
                        </div>
    ...
    // CRITICAL FIX: Form ko load hone ka time dein, fir submit attach karein
    if(isEditMode) {
        setTimeout(attachSubmit, 200); 
    }
}

// 3. FORM SUBMIT HANDLE KARNE WALA FUNCTION
